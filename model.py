...
class Model:
    ...
    def grid(self):
        ...
        k = abs(np.fft.fftfreq(self.n, d=self.a/(2*np.pi*self.n))[:int(self.n/2+1)])
    ...
    def advrhs(self, q):
        ...
        p = np.empty((self.l.size, self.k.size, self.nz), dtype=complex)
        for i in range(self.l.size):
            for j in range(self.k.size):
                p[i, j, :] = np.linalg.solve(self.L[i, j, :, :], q[i, j, :])
        # Calculate RHS.
        rhs = \
    ...
    def fft_truncate(self, up):
        ...
        u = np.zeros((self.n, int(self.n/2+1), self.nz), dtype=complex)
        u[: int(self.n/2), :, :] = us[: int(self.n/2), : int(self.n/2 + 1), :]
        u[int(self.n/2) :, :, :] = us[self.n : 3*int(self.n/2), : int(self.n/2 + 1), :]
        return u/2.25  # accounting for normalization

    def ifft_pad(self, u):
        """Pad spectral field u (3/2 rule) and perform inverse FFT."""
        us = np.zeros((3*int(self.n/2), 3*int(self.n/4+1), self.nz), dtype=complex)
        us[: int(self.n/2), : int(self.n/2+1), :] = u[: int(self.n/2), :, :]
        us[self.n : 3*int(self.n/2), : int(self.n/2+1), :] = u[int(self.n/2) :, :, :]
